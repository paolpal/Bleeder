...
def add_mirror_bleed(page, width, height, size, bleed):
    ...
    dx = abs(ratio*width-target_width)
    dy = abs(ratio*height-target_height)
    print(f"Resized Page Size: {target_width} x {target_height} points")
...
def modify_pdf(input_pdf_path, output_pdf_path, size, bleed):
    with open(input_pdf_path, 'rb') as file:
        ...
        for page in tqdm((reader.pages), desc="Processing Pages"):
            ...
            print(f"Original Page Size: {width} x {height} points")
            new = add_mirror_bleed(page, width, height, size, bleed)
            print(f"Final Page Size: {new.mediabox.width} x {new.mediabox.height} points")
            writer.add_page(new)
            gc.collect()
...
input_pdf_path = 'data/dark_left50.pdf'  # Inserire il percorso del PDF di input
